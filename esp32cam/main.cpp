...
void setup() {
    ...
    */
    ...
    digitalWrite(FLASH_LED, HIGH);  // solid on = WiFi up, attempting WS

    MDNS.begin("esp32-cam");
    server_on_response(onCamMessage);
    server_connect(WS_HOST, WS_PORT, "/cam");
}
...
void loop() {
    ...
    if (WiFi.status() != WL_CONNECTED) {
        ...
        return; // Halt until WiFi is back
    }
    ...
    if (connected != prevConnected) {
        ...
        digitalWrite(FLASH_LED, connected ? LOW : HIGH);  // off = running, on = disconnected
