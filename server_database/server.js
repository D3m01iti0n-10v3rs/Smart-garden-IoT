...
let isAutoMode = true; 
let currentRelayState = 0; // relay bit mask
...
function evaluateAutomation() {
  if (!isAutoMode) return; // check mode

  let nextState = currentRelayState;

  // fan 1 bit 0
  // temp
  if (latestSensors.temperature > autoConfig.tempMax) {
    nextState |= 1;  
  } else if (latestSensors.temperature < autoConfig.tempMin) {
    nextState &= ~1; 
  }

  // fan 2 bit 1
  // humi
  if (latestSensors.humidity > autoConfig.humidMax) {
    nextState |= 2;  
  } else if (latestSensors.humidity < autoConfig.humidMin) {
    nextState &= ~2; 
  }
  ...
  const isLightTime = autoConfig.lightStart < autoConfig.lightEnd
    ? (currentHour >= autoConfig.lightStart && currentHour < autoConfig.lightEnd)
    : (currentHour >= autoConfig.lightStart || currentHour < autoConfig.lightEnd);
  ...
  // pump bit 3
  // soil humi and water level
  const isTankEmpty = (latestSensors.water_level < autoConfig.minVolumeML);
  if (latestSensors.soil_humidity < autoConfig.soilMin && !isTankEmpty) {
    nextState |= 8;  
  } else if (latestSensors.soil_humidity > autoConfig.soilMax || isTankEmpty) {
    nextState &= ~8; 
  }

  // update state
  if (nextState !== currentRelayState) {
    currentRelayState = nextState;
    console.log(`[AUTO] State changed to: ${nextState}`);

    db.run(
      "INSERT INTO commands (cmd, value, timestamp) VALUES (?, ?, datetime('now', 'localtime'))",
      ["RLY", nextState],
      function (err) {
        if (!err && iotSocket && iotSocket.readyState === iotSocket.OPEN) {
          send(iotSocket, { type: "commands.new", id: this.lastID, cmd: "RLY", value: nextState });
        }
        broadcastUI({ type: "devices.changed", currentRelayState: nextState });
      }
    );
  }
}
        ...
        db.run(
          ...
          (err) => {
            if (err) {
              ...
              db.run(
                "DELETE FROM sensors WHERE id NOT IN (SELECT id FROM sensors ORDER BY id DESC LIMIT 259200)",
                (trimErr) => { if (trimErr) console.error("[DB] Trim error:", trimErr.message); }
              );

              if (latestSensors[msg.sensorType] !== undefined) {
                latestSensors[msg.sensorType] = msg.value;
              }
              // check to change device state
              evaluateAutomation(); 
            }
          }
        );
...
wss_ui.on("connection", (ws, req) => {
  ...
  uiClients.set(ws, { ip: clientIp, connectedAt: new Date().toLocaleTimeString() });

  console.log(`\n[UI] Client connected from ${clientIp}. Total: ${uiClients.size}`);
  logRemainingUiClients();

  // Send current auto settings to the GUI after connect
  send(ws, { type: "config.data", config: autoConfig, isAutoMode, currentRelayState });
  db.all("SELECT * FROM sensors ORDER BY id ASC", [], (err, rows) => {
    if (!err && rows.length) send(ws, { type: "sensors.data", rows });
  });
  ...
  ws.on("message", (data, isBinary) => {
    ...
    switch (msg.type) {
      ...
      case "commands.insert": {
        ...
        db.run(
          ...
          function (err) {
            ...
            if (msg.cmd === "RLY") currentRelayState = msg.value;

            if (iotSocket && iotSocket.readyState === iotSocket.OPEN) {
              send(iotSocket, { type: "commands.new", id: this.lastID, cmd: msg.cmd, value: msg.value });
            }
          }
        );
        break;
      ...
      // user update thresholds from gui
      case "config.update": {
        if (msg.thresholds) autoConfig = Object.assign(autoConfig, msg.thresholds);
        if (msg.isAutoMode !== undefined) isAutoMode = msg.isAutoMode;
        
        broadcastUI({ type: "config.data", config: autoConfig, isAutoMode, currentRelayState });
  ...
  mdns.on('query', function(query) {
    query.questions.forEach((q) => {
      if (q.name === 'gloriainexcelsisdeo.local' && q.type === 'A') {
        ...
        if (!ip) {
          console.warn('[mDNS] No local IP found, skipping response');
          return;
        }
        ...
        console.log(`[mDNS] Responded with local IP: ${ip}`);
      }
    });
  });
});