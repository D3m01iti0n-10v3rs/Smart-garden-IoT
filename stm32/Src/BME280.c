...
void BME280_Init(I2C_HandleTypeDef *hi2c, uint8_t addr){
    ...
    write_reg(0xF2, 0x01);  // humidity OSR x1
    write_reg(0xF4, 0x27);  // temp pressure OSR x1
...
BME280_Data BME280_Read(void){
    ...
    float var2 = (raw_t / 131072.0f - T1 / 8192.0f) * (raw_t / 131072.0f - T1 / 8192.0f) * T3;
    ...
    h = (raw_h - (H4 * 64.0f + H5 / 16384.0f * h)) * (H2 / 65536.0f * (1.0f + H6 / 67108864.0f * h * (1.0f + H3 / 67108864.0f * h)));
